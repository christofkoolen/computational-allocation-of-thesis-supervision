...
def normalize_topics(frame: pd.DataFrame) -> pd.DataFrame:
    """Return a one-row-per-topic canonical table."""
    ...
    issues.extend(_require_nonempty(result, ("topic_title",)))
    issues.extend(_validate_unique(result, "topic_title", "Topic titles"))

    supplied_ids = result["topic_id"].map(clean_text)
    if supplied_ids.eq("").all():
        result["topic_id"] = result["topic_title"].map(_slug)
    else:
        result["topic_id"] = supplied_ids
        issues.extend(_require_nonempty(result, ("topic_id",)))
...
def normalize_preferences(
    ...
    *,
    ...
    """Return canonical student preferences and apply an explicit duplicate policy."""
    ...
    for target, aliases in PREFERENCE_ALIASES.items():
        required = target in {"full_name", "email", "preference_1"}
        if not _copy_alias(result, target, aliases, required=required):
            issues.append(
    ...
    result["email"] = result["email"].map(normalize_email)
    for rank in (1, 2, 3):
        result[f"preference_{rank}"] = result[f"preference_{rank}"].map(clean_text)
    ...
    if duplicate_mask.any() and duplicate_policy == "error":
        ...
        issues.append(f"Student emails must be unique; duplicate value(s): {', '.join(emails)}")
    if issues:
        raise InputValidationError(issues)
...
def normalize_assignments(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    result["full_name"] = result["full_name"].map(clean_text)
    result["email"] = result["email"].map(normalize_email)
    result["assigned_topic_id"] = result["assigned_topic_id"].map(clean_text)
